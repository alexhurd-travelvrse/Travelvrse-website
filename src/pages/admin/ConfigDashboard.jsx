        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '2rem 1rem', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                ...
                <button
                    onClick={async () => {
                        try {
                            const response = await fetch('/api/save-full-manifest', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    companyId: activeWhitelabelId,
                                    manifestData: formData
                                })
                            });
                            const result = await response.json();
                            if (result.success) {
                                alert("🚀 SUCCESS: Manifest saved directly to project folder!\n\nYou can now push this change in GitHub Desktop.");
                            } else {
                                alert("Error: " + result.error);
                            }
                        } catch (e) {
                            console.error("Save error:", e);
                            alert("Failed to save to project. Make sure the local server is running.");
                        }
                    }}
                    style={{
                        padding: '1rem 2rem',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr', gap: '2rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    <Section title="Brand Identity" icon="🎨">
                        ...
                        <Field label="Mission Statement (Experience Page)" value={brandAssets.mission_statement} onChange={(v) => handleNestedChange('client_metadata', 'brand_assets', 'mission_statement', v)} />
                        ...
                        <Field label="Hero Video/Image URL" value={brandAssets.hero_image_url} onChange={(v) => handleNestedChange('client_metadata', 'brand_assets', 'hero_image_url', v)} />
                        <Field label="Hero Poster URL (Loading Image)" value={brandAssets.hero_poster_url} onChange={(v) => handleNestedChange('client_metadata', 'brand_assets', 'hero_poster_url', v)} />
                    ...
                    <Section title="Concierge & Proactive" icon="🤖">
                        <Field 
                            label="Music Concierge Question" 
                            value={formData.concierge_configuration?.music_question} 
                            onChange={(v) => handleNestedChange('concierge_configuration', null, 'music_question', v)} 
                        />
                        <Field 
                            label="Music Concierge Guide Unlocked (Text)" 
                            value={formData.concierge_configuration?.music_guide_text} 
                            onChange={(v) => handleNestedChange('concierge_configuration', null, 'music_guide_text', v)} 
                        />
                        <p style={{ fontSize: '0.7rem', opacity: 0.6, marginTop: '-0.5rem' }}>This is the agentic prompt that appears in the Digital Wallet after discovering music.</p>
                    </Section>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    <div style={{ backgroundColor: 'white', padding: '1.5rem', borderRadius: '16px', boxShadow: '0 4px 25px rgba(0,0,0,0.05)', border: '1px solid #f0f0f0' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            {(() => {
                                const experiences = formData.challenge_configuration?.experiences || 
                                              (formData.experiences ? Object.values(formData.experiences) : []);
                                    ...
                                    return (
                                        <div key={expIdx} style={{ padding: '1.5rem', border: '1px solid #eee', borderRadius: '12px', background: '#fafafa' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                                                ...
                                                <a 
                                                    href={`/experience/${exp.exp_id || expIdx + 1}?editor=true`} 
                                                    target="_blank" 
                                                    rel="noreferrer"
                                            ...
                                            <Field label="Thumbnail URL (Realistic Photo)" value={exp.thumbnail} onChange={(v) => handleExperienceChange(expIdx, 'thumbnail', v)} />
        
                                            <div style={{ background: 'white', padding: '1rem', borderRadius: '8px', border: '1px solid #eee', marginBottom: '1rem' }}>
                                                <h4 style={{ margin: 0, fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '1px', color: '#666', marginBottom: '1rem' }}>🎵 Music Player Configuration</h4>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                                                    <label style={{ fontSize: '0.8rem' }}>Active:</label>
                                                    <input 
                                                        type="checkbox" 
                                                        checked={exp.music_player?.active || false} 
                                                        onChange={(e) => {
                                                            const newExps = [...formData.challenge_configuration.experiences];
                                                            newExps[expIdx].music_player = { ...(newExps[expIdx].music_player || {}), active: e.target.checked };
                                                            setFormData(prev => ({ ...prev, challenge_configuration: { ...prev.challenge_configuration, experiences: newExps } }));
                                                        }}
                                                    />
                                                </div>
                                                {(exp.music_player?.tracks || []).map((track, tIdx) => (
                                                    <div key={tIdx} style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                                                        <input type="text" value={track.title} placeholder="Track Title" style={tableInput} onChange={(e) => {
                                                            const newExps = [...formData.challenge_configuration.experiences];
                                                            newExps[expIdx].music_player.tracks[tIdx].title = e.target.value;
                                                            setFormData(prev => ({ ...prev, challenge_configuration: { ...prev.challenge_configuration, experiences: newExps } }));
                                                        }} />
                                                        <input type="text" value={track.url} placeholder="Track URL" style={tableInput} onChange={(e) => {
                                                            const newExps = [...formData.challenge_configuration.experiences];
                                                            newExps[expIdx].music_player.tracks[tIdx].url = e.target.value;
                                                            setFormData(prev => ({ ...prev, challenge_configuration: { ...prev.challenge_configuration, experiences: newExps } }));
                                                        }} />
                                                    </div>
                                                ))}
                                                <button 
                                                    style={{ fontSize: '0.7rem', padding: '5px 10px', background: '#f0f0f0', border: '1px solid #ddd', borderRadius: '4px', cursor: 'pointer' }}
                                                    onClick={() => {
                                                        const newExps = [...formData.challenge_configuration.experiences];
                                                        if (!newExps[expIdx].music_player) newExps[expIdx].music_player = { active: true, tracks: [] };
                                                        newExps[expIdx].music_player.tracks.push({ title: "New Track", url: "" });
                                                        setFormData(prev => ({ ...prev, challenge_configuration: { ...prev.challenge_configuration, experiences: newExps } }));
                                                    }}
                                                >
                                                    + Add Track
