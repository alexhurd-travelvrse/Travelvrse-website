...
export const InfluencerProvider = ({ children }) => {
    ...
    const [activeWhitelabelId, setActiveWhitelabelId] = useState(() => {
        try {
            ...
            return saved || import.meta.env.VITE_WHITELABEL_ID || '25-hours-copenhagen';
        } catch (e) { return '25-hours-copenhagen'; }
    });

    const [manifest, setManifest] = useState(MANIFESTS[activeWhitelabelId] || MANIFESTS['25-hours-copenhagen']);

    useEffect(() => {
        const newManifest = MANIFESTS[activeWhitelabelId] || MANIFESTS['25-hours-copenhagen'];
    ...
    const publicConfig = useMemo(() => {
        ...
        // Map array or object experiences to indexed object for the engine
        const rawExperiences = manifest.challenge_configuration?.experiences || 
                               (manifest.experiences ? Object.values(manifest.experiences) : []);

        rawExperiences.forEach((exp, idx) => {
            const key = (exp.exp_id || idx + 1).toString();
            ...
            experiences[key] = {
                ...
                backpack_icons: items, // Keep raw icons for Scene3D
        ...
        return {
            ...
            home: {
                ...
                heroPoster: homeConfig.hero_poster_url,
