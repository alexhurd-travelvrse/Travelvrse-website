...
import { Billboard, Text, Float, PositionalAudio, Image, Html } from '@react-three/drei';
import { useFrame, useThree } from '@react-three/fiber';
import { Vector3 } from 'three';
import { useGame } from '../context/GameContext';
import { useInfluencer } from '../context/InfluencerContext';

const BackpackMarker = React.forwardRef(({ id, pos, size = 0.4, onClick, experienceId, isCollected, type, discoveryMode = 'instant', isActivity, audioUrl, isStarted, isModalOpen, isEditorMode }, ref) => {
    ...
    const [isMaterialized, setIsMaterialized] = useState(discoveryMode === 'instant' || isActivity);
    ...
    useEffect(() => {
        ...
        window.addEventListener('trigger-coaster-flip', (e) => {
            if (e.detail.id === id) setIsDismissed(true);
        });
    ...
    if (isCollected || isDismissed) return null;
    ...
    return (
        <group position={pos} ref={(el) => {
            ...
            >
                ...
                >
                    {isActivity && !isEditorMode && (
                        <Html transform center distanceFactor={10} zIndexRange={[0, 100]}>
                            <div 
                                className="pulse-activity" 
                                onClick={(e) => {
                                    e.stopPropagation();
                                    onClick(e);
                                }}
                                style={{ 
                                    ...
                                    boxShadow: `0 0 30px ${primaryColor}, 0 0 60px ${primaryColor}60`,
                                    display: 'flex',
                                    alignItems: 'center',
