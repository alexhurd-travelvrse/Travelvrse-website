...
export const usePlayerControls = (startPos, startRot = [0, 0, 0], boundaries = [], enabled = true) => {
    ...
    useEffect(() => {
        ...
        const handleJoystickMove = (e) => {
            ...
            // Lower deadzone threshold to 0.1 for higher responsiveness
            moveState.current.forward = y < -0.1;
            moveState.current.backward = y > 0.1;
            moveState.current.left = x < -0.1;
            moveState.current.right = x > 0.1;
            
            // Apply analog speed based on tilt intensity
            const intensity = Math.min(1.0, Math.sqrt(x*x + y*y));
            moveState.current.intensity = intensity;
        };
    ...
    useEffect(() => {
        const handleReset = () => {
            ...
            if (!startPos || !startRot) {
                console.warn("[usePlayerControls] Reset blocked: startPos or startRot missing");
                return;
            }
            camera.position.set(...(Array.isArray(startPos) ? startPos : [0, 2, 5]));
            camera.rotation.set(...(Array.isArray(startRot) ? startRot : [0, 0, 0]));
            if (velocity.current) velocity.current.set(0, 0, 0);
            if (rotationVelocity.current) rotationVelocity.current = { x: 0, y: 0 };
    ...
    useFrame((state, delta) => {
        ...
        if (forward || backward || left || right || up || down) {
            ...
            const intensity = moveState.current.intensity || 1.0;
            ...
            if (inputVector.lengthSq() > 0) {
                ...
                velocity.current.add(inputVector.multiplyScalar(ACCELERATION * intensity * delta));
            }
        }
