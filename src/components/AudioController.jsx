...
const AudioController = ({ audioKey, active, script }) => {
    ...
    // Activity Track Listener
    useEffect(() => {
        const handlePlayActivityTrack = (e) => {
            const { url } = e.detail;
            if (!url) return;
            
            console.log("[AudioController] 🎵 Playing activity track:", url);
            stopAll();
            playAudio(url, false);
        };

        window.addEventListener('play-activity-track', handlePlayActivityTrack);
        return () => window.removeEventListener('play-activity-track', handlePlayActivityTrack);
    }, []);
